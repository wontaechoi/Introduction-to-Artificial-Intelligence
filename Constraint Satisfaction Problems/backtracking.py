...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    if unAssignedVars.empty(): # no more assigning variables
        if trace: print "{} Solution Found".format(csp.name())
        solution = []
        for var in csp.variables():
            solution.append((var, var.getValue())) # append variables into solution list
        return [solution]
        ...
        for constraint in csp.constraintsOf(var):
            ...
            if constraint.numUnassigned() == 1:
                ...
                if FCCheck(constraint, var, val) == "DWO":
                    ...
                    break
        if noDWO:
            newSolution = FC(unAssignedVars,csp, allSolutions, trace)
            if newSolution:
                solution.extend(newSolution)
            if len(solution)> 0 and not allSolutions:
                var.restoreValues(var, val)
                break
        # restore values pruned by this assignment
        var.restoreValues(var, val)
...
def GacEnforce(constraints, csp, reasonVar, reasonVal):
    ...
    #constarints is a collection of constraints not known GAC.
...
def GAC(unAssignedVars, csp, allSolutions, trace):
    ...
    for val in var.curDomain():
        ...
        if GacEnforce(csp.constraintsOf(var),csp, var, val) == "DWO":
            ...
            noDWO = False
        if noDWO:
            newSolution = GAC(unAssignedVars,csp, allSolutions, trace)
            if newSolution:
                solution.extend(newSolution)

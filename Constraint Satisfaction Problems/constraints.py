...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        # initiate the constraint's name
        self._name = "Queen_" + name
        # Set the scope for the constraint's scope
        scope = [qi, qj]
        # initialize the satisfied assignments
        satAssignments= []
        for Qi in qi._dom:
            for Qj in qj._dom:
                # Check each domain to check satisfied indices
                if Qi != Qj and abs(Qi - Qj) != abs(i-j):
                    satAssignments.append([Qi, Qj])
                    satAssignments.append([Qj, Qi])
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        for variable in self.scope():
            if variable.getValue() in self._required:
                numVal +=1
        return self._lb <= numVal <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        def valsInBound(l):
            # check number of values are in the bound
            numVal = 0
            for (var, val) in l:
        ...
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        # use helper function findvals
        x = findvals(varsToAssign, [(var, val)], valsInBound)
        return x        

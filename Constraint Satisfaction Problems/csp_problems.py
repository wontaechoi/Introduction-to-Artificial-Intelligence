    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append(AllDiffConstraint("Subsquare" + str(i) + " " + str(j), scope ))
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    for solution in solutions:
        solution_list = []
        # make the first index of the list to be the plane
        for i in range(len(planes_problem.planes)):
            solution_list.append([planes_problem.planes[i]])
        # add each flight in the solution after the plane
        for (var, val) in solution:
            plane = int(var.name()[5])
            if val != 'no flight':
                solution_list[plane].append(val)
        converted_solution.append(solution_list)
    
    # return the solution
    return converted_solution


def planesCSP(planes_problem):
    ''' Planes problem converted into CSP problem.
    This function will build CSP problem on the planes problem.
    This function will make variables and constraints accordingly.
    Constraints will be following:
    1. Each plane is only assigned flights that it is capable of flying.
    2. Each plane's initial flight can only be a flight departing from that plane's initial location.
    3. The sequence of flights flown by every plane must be feasible. 
    4. Certain flights terminate at a maintenance location. All planes must be serviced with a certain minimum frequency. 
    5. Each flight must be scheduled. And no flight can be scheduled more than once.
    '''
    
    # variables from planes problem.
    ...
    for i in range(len(planes)):
        var_array.append([])
        can_fly = planes_problem.can_fly(planes[i])
        can_start = planes_problem.can_start(planes[i])
        # since each variable plane will have at most len(can_fly) flights to fly
        # Also checking the constraint #1
        for j in range(len(can_fly)):
            dom = []
            # If it is the first flight, check can_start constraint #2
            if j == 0:
                for flight in can_fly:
                    if flight in can_start:     
                        dom.append(flight)
                # add 'no flight' since no flight can be assigned.
                dom.append('no flight')
            else:
                # add all flight that plane can fly and 'no flight' option into domain
                for flight in can_fly:
                    dom.append(flight)
                    
                dom.append('no flight')
            # Make it as variable and add it to the variable array
            var = Variable('Plane{},{}'.format(i, j), dom)
            var_array[i].append(var)
    
    # The constraint list
    constraint_list = []
    
    # since Constraint #1 and #2 are already taken care of when we are initiating the variables,
    # the rest of the constraints will be added to the constraint list
    
    # constraint #3:
    # The sequence of flights flown by every plane must be feasible. 
    
    all_flight_sequence = []
    # Check all the possible flight sequence 
    for sequence in can_follow:
        all_flight_sequence.append(list(sequence))
    # add 'no flight' to 'no flight' option
    all_flight_sequence.append(['no flight', 'no flight'])
    # add a flight to 'no flight' option (termination flight)
    for flight in flights:
        all_flight_sequence.append([flight, 'no flight'])
            
    # For each plane
    for i in range(len(planes)):
        can_fly = planes_problem.can_fly(planes[i])
        for j in range(len(can_fly) - 1):
            # Get two flights that are scheduled in a row
            first_flight = var_array[i][j]
            second_flight = var_array[i][j + 1]
            # and check the constraint (if one flight can follow the other flight)
            constraint_list.append(TableConstraint("C3(F{}{}, F{}{})".format(i, j, i, j+1),
                                   [first_flight, second_flight], all_flight_sequence))
    
    # constraint #4
    # Certain flights terminate at a maintenance location. All planes must be serviced with a certain minimum frequency. 
    
    # for each plane
    for i in range(len(planes)):
        can_fly = planes_problem.can_fly(planes[i])
        for j in range(len(can_fly)):
            # check if the number of possible flight that the plane can fly exceeds the minimum maintenance frequency
            if len(can_fly) >= min_maintenance_frequency:
                # Get the last possible maintenance index
                last_index = j + min_maintenance_frequency -1
                # If last possible maintenance index is in the length of the possible flight schedule
                if last_index < len(can_fly):
                    # check the constraint using NValuesContraint
                    constraint_list.append(NValuesConstraint("C4", var_array[i][j:last_index+1], 
                                                             maintenance_flights + ['no flight'], 1, min_maintenance_frequency))   
                # if last possible maintenance index is greater, no need to take care of the constraint
                else: break  
            # if the minimum maintenance frequency is greater, do nothing since the constraint is already satisfied
            else:
                break
    ...
    # constraint #5:
    # Each flight must be scheduled. And no flight can be scheduled more than once.
    
    # Check if each flight is assigned in the variable list
    for flight in flights:
        constraint_list.append(NValuesConstraint("C5", vars, [flight], 1, 1))
        
    # return CSP
    return CSP("Planes", vars, constraint_list)

        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if nextState in self.corners:
                    corner = self.corners.index(nextState)
                    visitedCorner = list(state[2])
                    visitedCorner[corner] = 1
                    successors.append(((nextx, nexty, tuple(visitedCorner)), action, cost))
                else:
                    successors.append(((nextx, nexty, state[2]), action, cost))
...
def cornersHeuristic(state, problem):
    ...
    cornerList = []
    totalDistance = 0
    x, y, visitedCorner = state
    for i in range(0, 4):
        if visitedCorner[i] == 0:
            cornerList.append(i)
    while len(cornerList) != 0:
        distanceToCorner = 99999
        for i in cornerList:
        ...
        cornerList.remove(cornerIndex)
        x = corners[cornerIndex][0]
        y = corners[cornerIndex][1]
